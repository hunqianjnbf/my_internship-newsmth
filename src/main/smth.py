...
class ThreadViewWidget(QWidget):
    ...
    def create_post_widget(self, author: str, time: str, content: str, is_main_post: bool = False) -> QWidget:
        ...
        time_label.setStyleSheet("color: #7f8c8d; font-size: 16px;")
        
        # 如果是主贴，添加标识
        if is_main_post:
            main_label = QLabel("[主贴]")
            main_label.setStyleSheet("color: #e74c3c; font-weight: bold; font-size: 16px;")
        ...
        if content_length < 100:
            ...
            content_text.setMaximumHeight(220)
        else:
...
class PostWidget(QWidget):
    ...
    def init_ui(self):
        ...
        title_button.setCursor(Qt.PointingHandCursor)  # 鼠标悬停时显示手型光标
    ...
    def on_title_clicked(self):
        ...
        try:
            ...
            if hasattr(main_window, 'show_post_content'):
                ...
                if url:
                    print(f"点击标题: {title}, URL: {url}")
                    # 调用主窗口的显示帖子内容方法，传递URL参数
                    main_window.show_post_content(title, url)
                else:
                    print(f"帖子没有URL信息: {title}")
                    # 如果没有URL，仍然调用方法但只传递标题
