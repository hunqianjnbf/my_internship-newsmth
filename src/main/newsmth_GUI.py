...
class CrawlerWorker(QThread):
    ...
    content_fetched = pyqtSignal(dict)
    error_occurred = pyqtSignal(str)
    progress_updated = pyqtSignal(str)

    def __init__(self, crawler_type="posts", url=None, crawler_instance=None):
         super().__init__()
    ...
    def run(self):
         try:
             if self.crawler_type == "posts":
                 self.progress_updated.emit("正在获取预览区域帖子列表...")
                 posts_data = self.crawler_instance.fetch_posts(self.url)
                 if posts_data:
                     self.posts_fetched.emit(posts_data)
                 else:
                     self.error_occurred.emit("获取帖子列表失败")
             elif self.crawler_type == "content":
                 self.progress_updated.emit("正在获取帖子内容...")
                 content_data = self.crawler_instance.fetch_content(self.url)
                 if content_data:
                     self.content_fetched.emit(content_data)
                 else:
                     self.error_occurred.emit("获取帖子内容失败")
         except Exception as e:
             self.error_occurred.emit(f"爬虫操作失败:{str(e)}")
...
class window(QMainWindow):
    ...
    def __init__(self):
        ...
        # 初始化爬取内容的爬虫方法实例
        self.content_crawler = favorite_posts_content_craw()
        self.posts_data = []  # 存储当前版块的帖子数据
        self.crawler_worker = None
        self.posts_crawler = posts_preview_craw()
    ...
    def start_posts_crawler(self, url):
        print(f"开始启动帖子列表爬虫，URL: {url}")

        # 启动加载动画
        self.start_loading_animation("正在获取帖子列表...", 15)

        if self.crawler_worker and hasattr(self.crawler_worker, 'isRunning') and self.crawler_worker.isRunning():
            print("停止之前的爬虫工作器...")
            self.crawler_worker.quit()
            self.crawler_worker.wait()
            print("之前的爬虫工作器已停止")
        
        # 创建新的爬虫工作器
        print("创建新的帖子列表爬虫工作器...")
        self.crawler_worker = CrawlerWorker("posts", url, self.posts_crawler)
        
        # 连接信号
        self.crawler_worker.posts_fetched.connect(self.on_posts_fetched)
        self.crawler_worker.error_occurred.connect(self.on_crawler_error)
        self.crawler_worker.progress_updated.connect(self.on_crawler_progress)

        # 启动线程工作器
        self.crawler_worker.start()

        print(f"启动帖子内容爬虫,URL:{url}")

    def start_content_crawler(self, url):
        print(f"开始启动帖子内容爬虫，URL: {url}")
        
        # 启动加载动画
        self.start_loading_animation("正在获取帖子内容...", 20)
        ...
        self.crawler_worker = CrawlerWorker("content", url, self.content_crawler)

        self.crawler_worker.content_fetched.connect(self.on_content_fetched)
        self.crawler_worker.error_occurred.connect(self.on_crawler_error)
        self.crawler_worker.progress_updated.connect(self.on_crawler_progress)
    ...
    # 信号处理方法
    def on_posts_fetched(self, posts_data):
        try:
            posts = []
            ...
            # 停止加载动画
            self.stop_loading_animation("帖子列表获取完成")
        except Exception as e:
            print(f"处理帖子数据时出错: {e}")
            # 出错时显示错误状态
            self.show_error_loading("获取帖子列表失败")
            return

    def on_crawler_error(self, error_message):
        print(f"爬虫错误: {error_message}")
        # 显示加载错误状态
        self.show_error_loading("网络请求失败")
        QMessageBox.warning(self, "爬虫错误", "需要登录才能查看该帖子")

    def on_crawler_progress(self, progress_message):
        print(f"爬虫进度: {progress_message}")
        # 更新状态栏
    ...
    def on_content_fetched(self, posts_data):
        try:
            if not posts_data:
                print("获取失败")
                return
            ...
            if posts_data and "posts" in posts_data:
                ...
                for page_num in range(total_pages):
                    ...
                    page_layout.setContentsMargins(20, 0, 20, 20)
                    ...
                    for post_info in current_page_posts:
                        ...
                        post_layout.setContentsMargins(25, 25, 25, 25)
                        post_layout.setSpacing(18)
                ...
                if total_pages > 1:
                    ...
                    navigation_layout.setSpacing(15)
                ...
                # 停止加载动画
                self.stop_loading_animation("帖子内容获取完成")
        except Exception as e:
            print(f"处理帖子内容时出错: {e}")
            # 出错时显示错误状态
            self.show_error_loading("获取帖子内容失败")
    ...
    def init_ui(self):
        ...
        container.setContentsMargins(25, 5, 30, 25)
        ...
        self.setGeometry(300, 125, 2350, 1550)
        self.setStyleSheet("""
            QMainWindow {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
                    stop:0 #FAFAFA, stop:0.2 #E8F4FD, stop:0.4 #D1E9FB, stop:0.6 #E3F2FD, stop:0.8 #F0F8FF, stop:1 #FAFAFA);
            }
        """)
    ...
    def create_core(self):
        ...
        h_layout.setContentsMargins(25, 10, 25, 25)
    ...
    def create_right(self):
        ...
        toolbar_container_wrapper.setFixedHeight(130)
    ...
    def create_post_list(self):
        ...
        self.sort_combo.setFixedSize(350, 52)
        ...
        sort_control_layout.addSpacing(20)  # 调整间距，适应更宽的排序下拉框
    ...
    def create_left(self):
        ...
        content_layout.setSpacing(0)
        content_layout.setAlignment(QtCore.Qt.AlignTop)  # 设置顶部对齐，防止底部出现白色空间
        ...
        self.top10_layout.setContentsMargins(25, 25, 25, 25)
        self.top10_layout.setSpacing(25)

        # 楼主内容
        self.louzhu_layout = QVBoxLayout()
        self.louzhu_layout.setContentsMargins(25, 5, 25, 25)
        self.louzhu_layout.setSpacing(25)

        # 跟帖内容
        self.follower_layout = QVBoxLayout()
        self.follower_layout.setContentsMargins(25, 25, 25, 25)
        self.follower_layout.setSpacing(25)
        ...
        # 添加TOP10布局
        content_layout.addLayout(self.top10_layout, 1)
        content_layout.addSpacing(4)
        
        # 添加帖子内容布局
        content_layout.addLayout(self.louzhu_layout, 0)
        content_layout.addSpacing(8)
        content_layout.addLayout(self.follower_layout, 0)
        # 设置内容容器
    ...
    def start_loading_animation(self, message="加载中...", total_steps=15):
        try:
            ...
            # 设置动画参数
            self.loading_total_steps = total_steps
            self.loading_current_step = 0
            self.loading_step_size = 0.1
            self.loading_sleep_time = 10  # 毫秒 - 更快的更新频率实现更平滑动画
            ...
            self.progress_timer.start(self.loading_sleep_time)


            print(f"开始加载动画: {message}, 总步数: {total_steps}")
    ...
    def update_progress_animation(self):
        try:
            if hasattr(self, 'progress_fill') and hasattr(self, 'loading_current_step'):
                progress_bar_width = self.progress_bar.width()
                if progress_bar_width > 0:
                    progress_ratio = min(self.loading_current_step / self.loading_total_steps, 0.95)
                    smooth_progress = progress_ratio * (progress_bar_width - 2)
                    self.progress_fill.setFixedSize(int(smooth_progress), 6)
                
                # 使用更小的步长实现更平滑动画
                self.loading_current_step += 0.1
                
                # 检查是否完成
                if self.loading_current_step >= self.loading_total_steps:
                    self.progress_timer.stop()
                    
        except Exception as e:
            print(f"更新进度条动画时出错: {e}")


    def stop_loading_animation(self, message="完成"):
        try:
            # 停止定时器
            ...
            if hasattr(self, 'progress_fill'):
                ...
                if progress_bar_width > 0:
                    # 平滑滚动到终点
                    self.smooth_scroll_to_completion(progress_bar_width - 2)
                self.progress_fill.setStyleSheet("""
                    QWidget {
                        background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                            stop:0 #6495ED, stop:0.5 #0000CD, stop:1 #6495ED);
                        border-radius: 3px;
                        margin: 1px;
                    }
                """)
            
            print(f"加载动画完成: {message}")
            
        except Exception as e:
            print(f"停止加载动画时出错: {e}")

    def smooth_scroll_to_completion(self, target_width):
        try:
            if hasattr(self, 'progress_fill'):
                current_width = self.progress_fill.width()
                target_width = int(target_width)
                
                # 创建平滑滚动定时器
                if not hasattr(self, 'completion_timer'):
                    self.completion_timer = QTimer()
                    self.completion_timer.timeout.connect(self.update_completion_animation)
                
                # 设置动画参数
                self.completion_start_width = current_width
                self.completion_target_width = target_width
                self.completion_current_step = 0
                self.completion_total_steps = 20  # 20步完成从90%到100%
                
                # 启动平滑滚动动画
                self.completion_timer.start(15)  # 15ms更新频率
                
        except Exception as e:
            print(f"启动平滑滚动到完成时出错: {e}")

    def update_completion_animation(self):
        try:
            if hasattr(self, 'completion_current_step') and hasattr(self, 'completion_target_width'):
                # 计算当前宽度
                progress_ratio = self.completion_current_step / self.completion_total_steps
                current_width = int(self.completion_start_width + 
                                  (self.completion_target_width - self.completion_start_width) * progress_ratio)
                
                self.progress_fill.setFixedSize(current_width, 6)
                
                # 更新步数
                self.completion_current_step += 1
                
                # 检查是否完成
                if self.completion_current_step >= self.completion_total_steps:
                    self.completion_timer.stop()
                    # 确保最终显示100%
                    self.progress_fill.setFixedSize(self.completion_target_width, 6)
                    
                    # 1秒后开始淡出动画
                    QTimer.singleShot(400, self.start_fade_out_animation)
    ...
    def start_fade_out_animation(self):
        try:
            if hasattr(self, 'progress_bar') and hasattr(self, 'progress_fill'):
                # 创建淡出定时器
                if not hasattr(self, 'fade_out_timer'):
                    self.fade_out_timer = QTimer()
                    self.fade_out_timer.timeout.connect(self.update_fade_out_animation)
                
                # 设置淡出动画参数
                self.fade_out_current_step = 0
                self.fade_out_total_steps = 30  # 30步完成淡出
                self.fade_out_start_opacity = 1.0
                self.fade_out_target_opacity = 0.0
                
                # 启动淡出动画
                self.fade_out_timer.start(20)  # 20ms更新频率
    ...
    def update_fade_out_animation(self):
        try:
            if hasattr(self, 'fade_out_current_step'):
                # 计算当前透明度
                progress_ratio = self.fade_out_current_step / self.fade_out_total_steps
                current_opacity = self.fade_out_start_opacity + (self.fade_out_target_opacity - self.fade_out_start_opacity) * progress_ratio
                
                # 应用透明度到进度条和填充
                opacity_style = f"background-color: rgba(200, 200, 200, {current_opacity * 0.3});"
                self.progress_bar.setStyleSheet(f"""
                    QWidget {{
                        {opacity_style}
                        border-radius: 4px;
                        border: 1px solid rgba(33, 150, 243, {current_opacity * 0.2});
                    }}
                """)
                
                # 更新步数
                self.fade_out_current_step += 1
                
                # 检查是否完成淡出
                if self.fade_out_current_step >= self.fade_out_total_steps:
    ...
    def show_error_loading(self, error_message="加载失败"):
        try:
            # 停止定时器
            if hasattr(self, 'progress_timer'):
                self.progress_timer.stop()
            ...
            if hasattr(self, 'loading_status_label'):
                self.loading_status_label.setText(error_message)
            ...
            if hasattr(self, 'progress_fill'):
                progress_bar_width = self.progress_bar.width()
                if progress_bar_width > 0:
                    self.progress_fill.setFixedSize(progress_bar_width - 2, 6)
                self.progress_fill.setStyleSheet("""
                    QWidget {
                        background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                            stop:0 #F44336, stop:0.5 #EF5350, stop:1 #F44336);
                        border-radius: 3px;
                        margin: 1px;
                    }
                """)
            
            print(f"加载错误: {error_message}")
            
        except Exception as e:
            print(f"显示加载错误时出错: {e}")

    def show_post_content(self, url=None):
        try:
            if url:
                self.start_content_crawler(url)
                return
    ...
    def scroll_to_top(self):
        try:
            if hasattr(self, 'content_scroll_area'):
                self.content_scroll_area.verticalScrollBar().setValue(0)  # 滚动条设置为0
            else:
                print("未找到滚动区域")
        except Exception as e:
            print(f"滚动到顶部时出错: {e}")
    ...
    def add_image_to_layout(self, layout, img_info, post_type):
        ...
        if image_size[1] == 0:
            scaled_pixmap = pixmap.scaledToWidth(image_size[0], QtCore.Qt.SmoothTransformation)
        else:
    ...
    def show_top10_title(self):
        ...
        self.top10_layout.addSpacing(15)
    ...
    def create_top10_title_button(self, index, item):
        ...
        url = item['文章链接']
        title = item['标题']
        title_button.clicked.connect(lambda checked, u=url: self.top10_title_clicked(u))
        layout.addWidget(title_button)
        container.setLayout(layout)

        return container

    def top10_title_clicked(self, url):
        try:
            print(f"点击了top10标题，链接: {url}")
            self.show_post_content(url)
            return True
        ...
        def on_title_clicked(self):
            try:
                ...
                if hasattr(main_window, 'show_post_content'):
                    ...
                    if url:
                        main_window.show_post_content(url)
                    else:
                        main_window.show_post_content()
                else:
